...
from ..models.health import HealthStatus, HealthResponse, EndpointDetail, ModelStatus  # Updated models
...
@router.get("/", response_model=HealthResponse, summary="Comprehensive Health Check") # Changed response_model
async def health_check():
    ...
    for service_name, model_name_or_path in known_model_services.items():
        is_loaded = any(key.startswith(model_name_or_path) for key in model_manager._models.keys())
    ...
    endpoints = [
        EndpointDetail(path="/embed", method="POST", description="Single text embedding"),
        EndpointDetail(path="/embed/batch", method="POST", description="Batch text embeddings"),
        EndpointDetail(path="/translate", method="POST", description="German to English translation"),
        EndpointDetail(path="/summarize", method="POST", description="Text summarization"),
        EndpointDetail(path="/ner", method="POST", description="Named Entity Recognition"),
        EndpointDetail(path="/sentiment", method="POST", description="Sentiment analysis (single)"),
        EndpointDetail(path="/sentiment/batch", method="POST", description="Sentiment analysis (batch)"),
        EndpointDetail(path="/emotions", method="POST", description="Emotion analysis (single)"),
        EndpointDetail(path="/emotions/batch", method="POST", description="Emotion analysis (batch)"),
    ]
