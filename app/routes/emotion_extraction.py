...
from ..models.emotion_extraction import (
    EmotionInput,
    EmotionResponse,
    BatchEmotionInput,
...
async def analyze_emotions(emotion_input: EmotionInput, _=Depends(validate_api_key)):
    """
    ...
    try:
        ...
        raise HTTPException(
            status_code=500, detail=f"Error analyzing emotions: {str(e)}"
        )


@router.post("/emotions/batch", response_model=List[EmotionResponse])
async def analyze_emotions_batch(
    batch_input: BatchEmotionInput, _=Depends(validate_api_key)
):
    """
    ...
    try:
        ...
        raise HTTPException(
            status_code=500, detail=f"Error analyzing emotions batch: {str(e)}"
        )
