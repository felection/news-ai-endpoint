...
class TranslationService:
    def __init__(self):
        try:
            ...
            for direction, config in self.models.items():
                ...
                config["tokenizer"] = MarianTokenizer.from_pretrained(
                    config["model_name"]
                )
    ...
    def translate_in_chunks(
        self, text: str, max_chunk_size: int = 128, direction="de-en"
    ) -> str:
        """
        ...
        while start_index < num_tokens:
            ...
            if end_index < num_tokens:
                ...
                potential_chunk = tokenizer.decode(
                ...
                if (
                    last_terminator_pos > 0
                    and last_terminator_pos < len(potential_chunk) - 10
                ):
                    break_text = potential_chunk[: last_terminator_pos + 1]
                    break_tokens = tokenizer.encode(
                        break_text, add_special_tokens=False
                    )
            ...
            logger.info(
                f"Translating chunk {len(translated_chunks) + 1}: tokens {start_index} to {end_index - 1} ({end_index - start_index} tokens)"
            )
