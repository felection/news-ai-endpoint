...
class ModelManager:
    ...
    def get_model(
        ...
        **kwargs,
        ...
        if model_key not in self._models:
            try:
                logger.info(
                    f"Loading model: {model_name} (type: {model_type.__name__})"
                )
                ...
                if (
                    ...
                    and model_type != SentenceTransformer
                ):
    ...
    def _quantize_model(self, model):
        ...
        try:
            ...
            if hasattr(torch.quantization, "quantize_dynamic"):
                ...
                logger.warning(
                    "Dynamic quantization not available, using original model"
                )
        except Exception as e:
    ...
    def clear_cache(self, model_name: Optional[str] = None):
        ...
        if model_name:
            ...
            keys_to_clear = [
                k for k in self._models.keys() if k.startswith(f"{model_name}_")
            ]
    ...
    def get_memory_usage(self):
        ...
        if torch.cuda.is_available():
            memory_info["cuda_allocated_memory_mb"] = (
                torch.cuda.memory_allocated() / 1024 / 1024
            )
            memory_info["cuda_reserved_memory_mb"] = (
